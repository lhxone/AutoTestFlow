              ...
              <a-row :gutter="16">
                <a-col :span="24">
                  <a-form-item :label="t('testCase.detail.caseTitle')">
                    <a-input v-model:value="editableCase.title" />
                  </a-form-item>
                </a-col>
              </a-row>
              ...
              <a-row :gutter="16">
                <a-col :span="16">
                  <a-form-item :label="t('testCase.detail.precondition')">
                    <a-textarea v-model:value="editableCase.precondition" :rows="3" />
                  </a-form-item>
                </a-col>
                <a-col :span="8">
                ...
                <a-table :dataSource="editableCase.stepRows" :columns="stepColumns" :pagination="false" size="small" rowKey="key">
                  <template #bodyCell="{ column, record }">
                    <template v-if="column.key === 'step'">
                      <a-textarea v-model:value="record.step" :rows="2" />
                    </template>
                    <template v-if="column.key === 'expected'">
                      <a-textarea v-model:value="record.expected" :rows="2" />
                    </template>
                    <template v-if="column.key === 'action'">
                      <a-button type="link" danger size="small" @click="removeStepRow(editableCase.id, record.key)">
                        {{ t('common.delete') }}
                      </a-button>
                    </template>
                  </template>
                </a-table>
                <a-button type="dashed" block style="margin-top: 12px" @click="appendStepRow(editableCase.id)">
                  {{ t('testCase.detail.addStep') }}
                </a-button>
              </a-form-item>

              <a-form-item :label="t('testCase.detail.changeNote')" required>
                <a-input
        ...
        <a-collapse v-if="editableScripts.length">
          ...
          >
            <a-form layout="vertical">
              ...
              <a-button type="primary" :loading="savingScriptId === editableScript.id" @click="saveScript(editableScript)">
                {{ t('common.save') }}
              </a-button>
            </a-form>
          </a-collapse-panel>
...
import type { TestCaseVO, TestScriptVO, TestTask } from '@/types'
